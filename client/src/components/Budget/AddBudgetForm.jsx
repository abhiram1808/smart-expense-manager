 ...
 * It now uses a reusable CategoryDropdown component.
 */
const AddBudgetForm = ({ onAddBudget, disabled }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Client-side validation (more detailed validation in hook/backend)
    if (!formData.category || !formData.amount || !formData.month || !formData.year) {
      alert('Please fill in all required fields.'); // Use alert for now, toast in parent
      return;
    }
    const parsedAmount = parseFloat(formData.amount);
    if (isNaN(parsedAmount) || parsedAmount <= 0) {
      alert('Amount must be a positive number.'); // Use alert for now, toast in parent
      return;
    }
    ...
    try {
      ...
      // Error handling (toast) is done in the useBudget hook
          ...
          <div className="row g-3">
            <div className="col-md-3">
              ...
              <CategoryDropdown
                ...
                required
                disabled={disabled}
                // Assuming CategoryDropdown handles its own loading/error states internally
              />
            </div>
            <div className="col-md-3">
